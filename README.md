...
This is an errors crate with the purpose to provide better error debuggability in `async` call
stacks (backtraces are basically useless in `async` tasks). What we do is take advantage of
`#[track_caller]` and create a stack of `Location`s and errors via the helpful `StackableErr` trait.
